...
const PreCheckupFormModal = ({ isOpen, onClose, preCheckup, onSuccess, branch }) => {
    ...
    const { createPreCheckup, updatePreCheckup, loading: renderingForm } = usePreCheckup();
    ...
    const handleSubmit = async (e) => {
        ...
        const payload = {
            ...
            patient: selectedAppointment.patientId._id || selectedAppointment.patientId, // In case it's populated vs ID string
            branch,
        ...
        try {
            if (preCheckup?._id) {
                await updatePreCheckup(preCheckup._id, payload);
                    ...
                    {stage === 1 && (
                        <div className="animate-in slide-in-from-right-4 duration-300">
                            <div className="bg-gray-50 dark:bg-white/5 p-5 rounded-xl border border-gray-100 dark:border-white/5 mb-6">
                                <h4 className="font-semibold mb-4 text-casual-black dark:text-concrete">Step 1: Link Appointment & Patient</h4>
                                
                                <div className="join w-full grid grid-cols-3 mb-6">
                                    <button onClick={() => setLookupMode('appId')} className={`join-item btn btn-sm ${lookupMode === 'appId' ? 'btn-active bg-sporty-blue text-white' : ''}`}>By Appt ID</button>
                                    <button onClick={() => setLookupMode('phone')} className={`join-item btn btn-sm ${lookupMode === 'phone' ? 'btn-active bg-sporty-blue text-white' : ''}`}>By Mobile</button>
                                    <button onClick={() => setLookupMode('chamberDate')} className={`join-item btn btn-sm ${lookupMode === 'chamberDate' ? 'btn-active bg-sporty-blue text-white' : ''}`}>By Chamber & Date</button>
                                </div>
                                ...
                                <div className="flex gap-4 items-end">
                                    ...
                                    <button onClick={handleLookup} disabled={checkingAppt} className="btn btn-sm bg-fascinating-magenta hover:bg-fascinating-magenta/90 text-white flex items-center gap-2">
