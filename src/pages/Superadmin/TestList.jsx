...
import useLabTestDept from '../../Hook/useLabTestDept'; // ADDED: Import Department hook
...
const TestList = () => {
  ...
  const [departments, setDepartments] = useState([]); // ADDED: To store dynamic dropdown options
  ...
  const { getPaginatedLabTestDepts } = useLabTestDept(); // ADDED: Extract fetch function
  ...
  useEffect(() => {
    const fetchDepartments = async () => {
      try {
        const response = await getPaginatedLabTestDepts({ limit: 100 }); // Fetch enough to populate dropdown
        if (response && response.success) {
          // Extract just the department names into a simple array
  ...
  const fetchLabtestsData = useCallback(async () => {
    try {
      const response = await getPaginatedLabtests({
        ...
        department: departmentFilter || undefined // ADDED: Pass department to API
      });
  ...
  }, [page, limit, searchTerm, departmentFilter, getPaginatedLabtests]); // ADDED: Added departmentFilter to dependency array

  // Triggers dynamic search due to dependency array
  useEffect(() => {
    fetchLabtestsData();
  }, [fetchLabtestsData]);
  ...
  return (
    <div className="p-4 md:p-6 bg-base-100 dark:bg-casual-black min-h-screen font-primary text-casual-black dark:text-concrete transition-colors">
      ...
      <div className="bg-concrete dark:bg-white/5 p-4 rounded-box shadow-sm mb-6 flex flex-col md:flex-row items-center gap-4 border border-casual-black/5 dark:border-white/10 transition-colors">
        ...
        {/* Clear Filters (Updated to include departmentFilter) */}
        {(searchTerm || departmentFilter) && (
          <button 
            onClick={() => { setSearchTerm(''); setDepartmentFilter(''); setPage(1); }}
      ...
      <LabtestFormModal 
        ...
        departments={departments} // Passing it here so your modal can use the dropdown too!
      />
      <ConfirmDeleteModal 
